...
import {
	buildAuthRouteTemplateMatcher,
	getAuthRouteBasePath,
	getAuthWildcardRouteTemplate,
	matchAuthRouteTemplate,
} from "./route-template";
...
describe("better-auth route-template helpers", () => {
	...
	it("returns wildcard template and matched endpoint metadata", () => {
		...
		expect(getAuthWildcardRouteTemplate(authLike)).toBe("/api/auth/*");
